...
/**
 * This class creates the grid with the five ships for one player. It checks
 * each ship if is being initialised at a valid position and if not it changes
 * its position until is valid, with the help of the Coordinates.
 * 
 * @author Ken Liam
 * @date 01/01/2024
 *
...
public class Ocean {
	 ...
	 * uses the Coordinates classes to get new position until is valid {@link Ship}
	 * 
	 * @param sh
	 */
	public void initializeShip(Ship sh) {
		...
		if (HorOrVer == 0) {
			boolean check = false;
			// check if the position is valid,else generate randomly new position until is
			// valid
			while (check == false) {
